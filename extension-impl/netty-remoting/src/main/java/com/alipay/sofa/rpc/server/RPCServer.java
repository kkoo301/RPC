...
public class RPCServer extends AbstractServer {
    ...
    private final EventLoopGroup bossGroup = NettyEventLoopUtil.newEvenLoopGroup(1, new ThreadFactoryBuilder().setNameFormat("boss-thread-%s").setDaemon(false).build());

    private final EventLoopGroup workerGroup = NettyEventLoopUtil.newEvenLoopGroup(Runtime.getRuntime().availableProcessors() * 2, new ThreadFactoryBuilder().setNameFormat("worker-thread-%s").setDaemon(false).build());
