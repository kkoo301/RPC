...
public class NetUtils {
  ...
  public static int getAvailablePort(String host, int port, int maxPort) {
    if (isAnyHost(host) || isLocalHost(host) || isHostInNetworkCard(host)) {
      ...
      throw new SofaRpcRuntimeException(
          "The host " + host + " is not found in network cards, please check config");
    }
  }
  ...
  private static InetAddress getLocalHostBySocket(InetSocketAddress remoteAddress) {
    ...
    try {
      ...
      LOGGER.warn(
          "Can not connect to host {}, cause by :{}", remoteAddress.toString(), e.getMessage());
    }
    return host;
  }
    ...
    for (int j = 0; j < ips.size(); j++) {
      ...
      try {
        InetSocketAddress address =
            new InetSocketAddress(ip[0], Integer.parseInt(ip[1] == null ? defaultPort : ip[1]));
        ads.add(address);
