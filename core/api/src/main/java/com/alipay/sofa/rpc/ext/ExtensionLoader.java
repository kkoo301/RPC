...
public class ExtensionLoader<T> {
  ...
  public ExtensionLoader(
      Class<T> interfaceClass, boolean autoLoad, ExtensionLoaderListener<T> listener) {
    ...
    if (autoLoad) {
      List<String> paths =
          RpcConfigs.getOrDefaultValue(RpcOptions.EXTENSION_LOAD_PATH, Lists.newArrayList());
      ...
      if (extension.override()) {
        if (extension.order() < old.getOrder()) {
          ...
          if (LOGGER.isInfoEnabled()) {
            LOGGER.info(
                "Extension of extensible {} with alias {}: {} has been override to {}",
                interfaceName,
                alias,
                old.getClazz(),
                implClass);
        ...
        if (old.isOverride() && old.getOrder() >= extension.order()) {
          ...
          if (LOGGER.isInfoEnabled()) {
            LOGGER.info(
                "Extension of extensible {} with alias {}: {} has been loaded, ignore origin {}",
                interfaceName,
                alias,
                old.getClazz(),
                implClass);
          ...
          throw new IllegalStateException(
              "Error when load extension of extensible "
                  ...
                  + " from file:"
                  + fileUrl
                  + ", Duplicate class with same alias: "
                  + alias
                  + ", "
                  + old.getClazz()
                  + " and "
  ...
  private ExtensionClass buildClass(
